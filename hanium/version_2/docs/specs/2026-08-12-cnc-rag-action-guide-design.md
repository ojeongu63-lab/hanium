...
| 항목 | 내용 |
|---|---|
| 의존성 추가 | `openai`(Python SDK), `faiss-cpu` — `version_2/pyproject.toml` |
...
| `version_2/src/rag/generation.py` | 신규 — OpenAI 호출 + 프롬프트 |
| `version_2/src/serving/inference.py` | 수정 — `predict_experiment()`가 `guide` 필드 포함하도록 확장 |
| `version_2/src/serving/app.py` | 수정 — 기동 시 코퍼스/인덱스 로드, `OPENAI_API_KEY` 확인 |
| `version_2/pyproject.toml` | 수정 — `openai`, `faiss-cpu` 의존성 추가 |

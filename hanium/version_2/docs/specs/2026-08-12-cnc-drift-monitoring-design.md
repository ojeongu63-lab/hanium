...
해결했다. CNC(version_2)에는 이런 보정 로직이 전혀 없다(`lstm_ae/pipeline.py`
...
**스크립트**: `version_2/monitoring/simulate_drift.py`(일회성 검증 스크립트,
...
| 파일 | 변경 |
|---|---|
| `version_2/src/monitoring/logging.py` | 신규 |
| `version_2/src/monitoring/drift.py` | 신규 |
| `version_2/src/serving/app.py` | 수정 — `/predict`에 로그 기록 추가, `GET /drift-status` 신규 |
| `version_2/monitoring/simulate_drift.py` | 신규 — 검증 스크립트 |
| `version_2/data/monitoring/` | 신규 — `.gitignore` 대상(`data/` 관례) |

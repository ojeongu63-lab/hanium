...
- 각 시나리오는 원본 실험 CSV와 완전히 같은 형식(48컬럼)이라 `/predict`에
  수정 없이 바로 업로드할 수 있다.
- 각 시나리오가 실제로 모델에 의해 "불량"으로 판정되고, `feature_contributions`
  상위권에 그 시나리오가 의도한 피처가 나오는지 **직접 검증**한다(임의로 만든
  값이 실제로 의도대로 작동하는지 확인 없이 넘기지 않는다).
...
```
version_2/synthetic/
  generate_synthetic.py         # 3개 시나리오 생성 + 자동 보정 루프 + 검증
  scenarios/
    tool_wear.csv                   # 시나리오 ①
    tool_wear_predict_result.json   # 그 /predict 응답(검증 기록)
    feed_overload.csv               # 시나리오 ②
    feed_overload_predict_result.json
    vibration_backlash.csv          # 시나리오 ③
...
`version_2/synthetic/`에 계속 남겨두는 재사용 가능한 데모 자산이다.

## 코드 변경 요약

| 파일 | 변경 |
|---|---|
| `version_2/synthetic/generate_synthetic.py` | 신규 |
| `version_2/synthetic/scenarios/*.csv`, `*.json` | 신규 (생성 산출물) |
| 기존 `src/` 코드 | 변경 없음 — `serving.inference.predict_experiment()`,
  `serving.app.load_model_state()`를 그대로 재사용 |

## 테스트 범위

정식 pytest 단위테스트는 만들지 않는다 — 위 자동 보정 루프 자체가 "생성된
각 시나리오가 실제로 bad로 판정되는지" 검증하는 런타임 체크 역할을 한다
(이 프로젝트의 일회성 스크립트 관례와 동일).

## 검증 방법

1. `generate_synthetic.py` 실행 → 3개 시나리오 전부 5회 이내에 `"bad"` 판정을
   받는지 확인(로그로 몇 번째 시도에서 성공했는지 출력).
2. 각 `*_predict_result.json`의 `feature_contributions` 상위 3개가 그 시나리오의
   의도된 대상 피처와 겹치는지 육안 확인 — 안 겹치면(엉뚱한 피처가 1등이면)
   시나리오 설계 자체를 재검토(다음 세션에서 논의).

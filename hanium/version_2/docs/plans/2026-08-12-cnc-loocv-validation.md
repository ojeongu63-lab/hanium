...
**Architecture:** 기존 `preprocessing.pipeline.run_pipeline()`과 `lstm_ae.pipeline.run_lstm_pipeline()`을 수정 없이 재사용해, 정상 11개를 1개씩 held-out하며 11번 반복 호출하는 오케스트레이션 스크립트 하나만 새로 작성한다. 결과는 `version_2/loocv/`에 기존 산출물과 완전히 분리해서 저장한다.

**Tech Stack:** Python (uv run), pandas, PyTorch(CPU) — 전부 기존 `version_2` 의존성 그대로, 새 패키지 추가 없음.
...
- Create: `version_2/loocv/run_loocv.py` — 오케스트레이션 스크립트 (fold 반복 실행 + 집계 + summary 저장)
- Create: `version_2/loocv/.gitignore` — `folds/`(재생성 가능한 대용량 산출물) 제외
- Generated at runtime (git 제외): `version_2/loocv/folds/fold_<id>/processed/`, `version_2/loocv/folds/fold_<id>/model/`
- Generated at runtime (git 포함): `version_2/loocv/summary.csv`, `version_2/loocv/summary.json`

## Task 1: `run_loocv.py` 작성 + 문법 확인

**Files:**
- Create: `version_2/loocv/run_loocv.py`
- Create: `version_2/loocv/.gitignore`
...
- Produces: `version_2/loocv/summary.csv` (11행), `version_2/loocv/summary.json` (`good_side_loocv`, `bad_side_stability`, `comparison_to_fixed_split` 키)

- [ ] **Step 1: `version_2/loocv/.gitignore` 작성**

```
folds/
```

- [ ] **Step 2: `version_2/loocv/run_loocv.py` 작성**
...
Run: `cd version_2 && uv run python -m py_compile loocv/run_loocv.py`
Expected: 에러 없이 종료 (exit code 0)

- [ ] **Step 4: Commit**

```bash
git add version_2/loocv/run_loocv.py version_2/loocv/.gitignore
...
Run:
```bash
cd version_2 && who && top -bn1 | head -15
...
"
```
Expected: `who`/`top`으로 서버 여유 확인 후, assertion 없이 통과하고 `{'precision': ..., 'recall': ..., 'tp': ..., 'fp': ..., 'fn': ..., 'tn': ...}` 형태 출력. `version_2/loocv/folds/fold_01/`에 `processed/`, `model/` 디렉토리가 생성됨을 `ls`로 확인.
...
Run: `cd version_2 && who && top -bn1 | head -15 && nice -n 19 uv run python loocv/run_loocv.py`
...
```bash
cd version_2 && git add loocv/summary.csv loocv/summary.json

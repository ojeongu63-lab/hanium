...
모델이 "이 구간의 정상"을 학습할 근거 자체가 부족합니다. 이를 보완하려 **소규모
증강**을 구현해 LOOCV로 검증했으나 오히려 악화됐습니다(정상 홀드아웃 9/11 →
8/11). OOD 게이트와 LOOCV 폴드 앙상블은 설계 단계에서 다음 서브프로젝트로
유예되어 구현되지 않았습니다. 원인이 모델이나 하이퍼파라미터가 아니라
**해당 조건의 표본 부족**임이 확인돼, 무리하게 맞추는 대신 알려진 한계로
...
이 LSTM-Autoencoder 이상탐지 방법론은 원래 다른 제조공정 데이터에 먼저
적용해봤으나, 평가셋의 진짜 불량이 18건뿐이라 성능 지표를 신뢰하기 어려웠습니다.
그래서 **같은 방법론을 불량 표본이 확보된 CNC 데이터에 재적용해 유효성을
검증**했고(precision 0.91 / recall 0.91), 거기에 실제 운용에 필요한 MLOps와
RAG를 얹은 것이 이 파이프라인입니다.

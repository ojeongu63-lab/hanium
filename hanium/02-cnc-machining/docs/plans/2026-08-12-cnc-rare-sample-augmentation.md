...
- `02-cnc-machining/augmentation/combined_dataset/`, `combined_index.csv`, `loocv/augmented_folds/`는 재생성 가능한 산출물이라 `.gitignore` 대상. `loocv/summary_augmented.json`, `summary_augmented.csv`는 결과 증거 자료라 git 포함.
...
- Create: `02-cnc-machining/augmentation/generate_augmented.py`
- Create: `02-cnc-machining/augmentation/.gitignore` (`combined_dataset/`, `combined_index.csv` 제외)
- Create: `02-cnc-machining/loocv/run_loocv_augmented.py`
- Create: `02-cnc-machining/loocv/.gitignore`에 `augmented_folds/` 추가(기존 `.gitignore` 수정)

## Task 1: `generate_augmented.py` 작성 + 실행 + 검증

**Files:**
- Create: `02-cnc-machining/augmentation/generate_augmented.py`
- Create: `02-cnc-machining/augmentation/.gitignore`

**Interfaces:**
- Consumes: `preprocessing.columns.FEATURE_COLUMNS`
- Produces: `02-cnc-machining/augmentation/combined_dataset/experiment_{01..25,2001..2003,2201..2203}.csv`,
  `02-cnc-machining/augmentation/combined_index.csv`(31행)

- [ ] **Step 1: `.gitignore` 작성**

`02-cnc-machining/augmentation/.gitignore`:
```
combined_dataset/
combined_index.csv
```
...
Run: `cd 02-cnc-machining && uv run python -m py_compile augmentation/generate_augmented.py`
Expected: 에러 없이 종료

- [ ] **Step 4: 실행**

Run: `cd 02-cnc-machining && uv run python augmentation/generate_augmented.py`
...
```bash
git add 02-cnc-machining/augmentation/generate_augmented.py 02-cnc-machining/augmentation/.gitignore
...
**Files:**
- Create: `02-cnc-machining/loocv/run_loocv_augmented.py`
- Modify: `02-cnc-machining/loocv/.gitignore`

**Interfaces:**
- Consumes: Task 1의 `02-cnc-machining/augmentation/combined_dataset/`, `combined_index.csv`;
  `preprocessing.pipeline.run_pipeline()`, `lstm_ae.pipeline.run_lstm_pipeline()`(기존 loocv와 동일 시그니처)
- Produces: `02-cnc-machining/loocv/summary_augmented.json`, `summary_augmented.csv`

- [ ] **Step 1: `.gitignore`에 `augmented_folds/` 추가**

`02-cnc-machining/loocv/.gitignore`에 `folds/` 다음 줄에 추가:
```
folds/
augmented_folds/
```
...
Run: `cd 02-cnc-machining && uv run python -m py_compile loocv/run_loocv_augmented.py`
Expected: 에러 없이 종료
...
```bash
git add 02-cnc-machining/loocv/run_loocv_augmented.py 02-cnc-machining/loocv/.gitignore \
        02-cnc-machining/loocv/summary_augmented.json 02-cnc-machining/loocv/summary_augmented.csv

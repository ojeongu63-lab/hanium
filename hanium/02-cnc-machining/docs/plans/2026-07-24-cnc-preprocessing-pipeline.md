...
**Goal:** `02-cnc-machining/data/dataset/CNC 비식별화 원본데이터_1209`의 원본 25개 실험(`train.csv` +
...
`02-cnc-machining/data/processed/`에 만든다.

**Architecture:** hanium 루트의 `src/preprocessing/` 모듈 스타일(단일 책임 소파일 + 얇은
`pipeline.py` 오케스트레이터)을 `02-cnc-machining/src/preprocessing/` 아래 독립적으로 복제한다.
...
- pytest 설정: `testpaths = ["tests"]`, `addopts = "--import-mode=importlib"` — 테스트
  파일에 `__init__.py` 불필요.
- `02-cnc-machining/` 아래 코드만 사용. hanium 루트 `src/`를 import하지 않는다.
...
**Files:**
- Create: `02-cnc-machining/pyproject.toml`
- Create: `02-cnc-machining/src/preprocessing/__init__.py` (빈 파일)
- Create: `02-cnc-machining/src/preprocessing/columns.py`
- Create: `02-cnc-machining/tests/preprocessing/test_columns.py`
...
- [ ] **Step 1: `02-cnc-machining/pyproject.toml` 작성**
...
- [ ] **Step 2: 빈 `02-cnc-machining/src/preprocessing/__init__.py` 생성**

- [ ] **Step 3: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_columns.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_columns.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing'` (아직 `columns.py`가 없음)

- [ ] **Step 5: `02-cnc-machining/src/preprocessing/columns.py` 구현**
...
```bash
git add 02-cnc-machining/pyproject.toml 02-cnc-machining/src/preprocessing/__init__.py 02-cnc-machining/src/preprocessing/columns.py 02-cnc-machining/tests/preprocessing/test_columns.py
```
...
**Files:**
- Create: `02-cnc-machining/src/preprocessing/labels.py`
- Create: `02-cnc-machining/tests/preprocessing/test_labels.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_labels.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_labels.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing.labels'`

- [ ] **Step 3: `02-cnc-machining/src/preprocessing/labels.py` 구현**
...
```bash
git add 02-cnc-machining/src/preprocessing/labels.py 02-cnc-machining/tests/preprocessing/test_labels.py
```
...
**Files:**
- Create: `02-cnc-machining/src/preprocessing/cleaning.py`
- Create: `02-cnc-machining/tests/preprocessing/test_cleaning.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_cleaning.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_cleaning.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing.cleaning'`

- [ ] **Step 3: `02-cnc-machining/src/preprocessing/cleaning.py` 구현**
...
```bash
git add 02-cnc-machining/src/preprocessing/cleaning.py 02-cnc-machining/tests/preprocessing/test_cleaning.py
```
...
**Files:**
- Create: `02-cnc-machining/src/preprocessing/split.py`
- Create: `02-cnc-machining/tests/preprocessing/test_split.py`
...
- Produces: `TRAIN_EXPERIMENT_IDS: list[int]`(8개), `EVAL_GOOD_EXPERIMENT_IDS: list[int]`(5개),
  ...
  가져다 쓴다. 스펙 문서(`02-cnc-machining/docs/specs/2026-07-24-cnc-preprocessing-pipeline-design.md`)
  의 "Train / Eval 분할" 표와 정확히 일치해야 한다.

- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_split.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_split.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing.split'`

- [ ] **Step 3: `02-cnc-machining/src/preprocessing/split.py` 구현**
...
```bash
git add 02-cnc-machining/src/preprocessing/split.py 02-cnc-machining/tests/preprocessing/test_split.py
```
...
**Files:**
- Create: `02-cnc-machining/src/preprocessing/scaling.py`
- Create: `02-cnc-machining/tests/preprocessing/test_scaling.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_scaling.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_scaling.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing.scaling'`

- [ ] **Step 3: `02-cnc-machining/src/preprocessing/scaling.py` 구현**
...
```bash
git add 02-cnc-machining/src/preprocessing/scaling.py 02-cnc-machining/tests/preprocessing/test_scaling.py
```
...
**Files:**
- Create: `02-cnc-machining/src/preprocessing/manifest.py`
- Create: `02-cnc-machining/tests/preprocessing/test_manifest.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_manifest.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_manifest.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing.manifest'`

- [ ] **Step 3: `02-cnc-machining/src/preprocessing/manifest.py` 구현**
...
```bash
git add 02-cnc-machining/src/preprocessing/manifest.py 02-cnc-machining/tests/preprocessing/test_manifest.py
```
...
**Files:**
- Create: `02-cnc-machining/src/preprocessing/pipeline.py`
- Create: `02-cnc-machining/tests/preprocessing/test_pipeline.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/preprocessing/test_pipeline.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/preprocessing/test_pipeline.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'preprocessing.pipeline'`

- [ ] **Step 3: `02-cnc-machining/src/preprocessing/pipeline.py` 구현**
...
Run: `cd 02-cnc-machining && uv run pytest -v`
Expected: 모든 테스트 PASS (Task 1~7 누적, 총 16개)

- [ ] **Step 6: 스테이징**

```bash
git add 02-cnc-machining/src/preprocessing/pipeline.py 02-cnc-machining/tests/preprocessing/test_pipeline.py
```
...
**Files:**
- Create: `02-cnc-machining/scripts/run_preprocessing.py`

**Interfaces:**
- Consumes: Task 4의 `TRAIN_EXPERIMENT_IDS`/`EVAL_GOOD_EXPERIMENT_IDS`/`EVAL_BAD_EXPERIMENT_IDS`,
  Task 7의 `run_pipeline`.
- Produces: 없음 (스크립트, 실행하면 `02-cnc-machining/data/processed/`에 4개 파일 생성)

- [ ] **Step 1: `02-cnc-machining/scripts/run_preprocessing.py` 작성**
...
Run: `cd 02-cnc-machining && uv run python scripts/run_preprocessing.py`

- [ ] **Step 3: 출력을 스펙 문서의 실측값과 대조 (수동 검증)**

`manifest.json`(또는 stdout)에서 아래 값이 스펙(`02-cnc-machining/docs/specs/2026-07-24-cnc-preprocessing-pipeline-design.md`)과 정확히 일치하는지 확인한다:
...
Run: `ls -la 02-cnc-machining/data/processed/`
Expected: `train.csv`, `eval.csv`, `scaler.json`, `manifest.json` 4개 파일 존재

- [ ] **Step 5: 스테이징**

```bash
git add 02-cnc-machining/scripts/run_preprocessing.py
```

(`02-cnc-machining/data/processed/`는 루트 `.gitignore`의 `data` 패턴에 이미 걸려 자동 제외됨 —
확인됨: `git check-ignore -v 02-cnc-machining/data/dataset`)

---

## 완료 후 체크리스트

- [ ] `cd 02-cnc-machining && uv run pytest -v` 전체 통과 (16개 유닛/통합 테스트)

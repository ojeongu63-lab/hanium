...
- MLflow tracking/registry URI: `sqlite:///{ROOT}/data/mlflow/mlflow.db` (`ROOT` = `02-cnc-machining/`), 아티팩트 위치: `file://{ROOT}/data/mlflow/artifacts`. `data/`는 이미 `.gitignore` 대상.
...
- 모든 신규/변경 코드는 `cd 02-cnc-machining && uv run pytest tests/ -v`로 검증한다.
...
- Test: `02-cnc-machining/tests/test_mlops_dependencies.py`
...
```python
# 02-cnc-machining/tests/test_mlops_dependencies.py
...
Run: `cd 02-cnc-machining && uv run pytest tests/test_mlops_dependencies.py -v`
...
Run: `cd 02-cnc-machining && uv run pytest tests/ -v`
...
**Files:**
- Modify: `02-cnc-machining/src/lstm_ae/pipeline.py:112-118`
- Modify: `02-cnc-machining/tests/lstm_ae/test_pipeline.py:7`, `:88-90`
...
In `02-cnc-machining/tests/lstm_ae/test_pipeline.py`, add the import after line 7 (`from lstm_ae.pipeline import run_lstm_pipeline`):
...
In `02-cnc-machining/src/lstm_ae/pipeline.py`, replace the final return statement (lines 112-118):
...
**Files:**
- Create: `02-cnc-machining/src/lstm_ae/tracking.py`
- Test: `02-cnc-machining/tests/lstm_ae/test_tracking.py`
...
```python
# 02-cnc-machining/tests/lstm_ae/test_tracking.py
import json
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_tracking.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'lstm_ae.tracking'`

- [ ] **Step 3: Write minimal implementation**

```python
# 02-cnc-machining/src/lstm_ae/tracking.py
import json
...
**Files:**
- Modify: `02-cnc-machining/scripts/run_lstm_training.py` (전체 재작성)
...
`02-cnc-machining/scripts/run_lstm_training.py`의 기존 내용을 아래로 전체 교체한다:
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/ -v`
...
```bash
cd 02-cnc-machining && uv run python -c "
...
**Files:**
- Modify: `02-cnc-machining/src/lstm_ae/tracking.py` (append)
- Modify: `02-cnc-machining/tests/lstm_ae/test_tracking.py` (append)
- Create: `02-cnc-machining/scripts/promote_model.py`
...
Append to `02-cnc-machining/tests/lstm_ae/test_tracking.py` (add these imports to the top alongside the existing ones, and the new test at the end):
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_tracking.py::test_promote_to_champion_sets_alias -v`
Expected: FAIL — `ImportError: cannot import name 'promote_to_champion'`

- [ ] **Step 3: Write minimal implementation**

Append to `02-cnc-machining/src/lstm_ae/tracking.py`:
...
```python
# 02-cnc-machining/scripts/promote_model.py
import argparse
...
Run: `cd 02-cnc-machining && uv run python scripts/promote_model.py 1`
...
**Files:**
- Modify: `02-cnc-machining/src/lstm_ae/pipeline.py:18`, `:60-61` (rename `_compute_window_errors` → `compute_window_errors`)
- Create: `02-cnc-machining/src/serving/__init__.py` (빈 파일)
- Create: `02-cnc-machining/src/serving/inference.py`
- Test: `02-cnc-machining/tests/serving/test_inference.py`
...
`02-cnc-machining/src/lstm_ae/pipeline.py`에서 `_compute_window_errors` 정의(18번 줄)와 두 호출부(60-61번 줄)의 이름을 `compute_window_errors`로 바꾼다:
...
```python
# 02-cnc-machining/tests/serving/test_inference.py
import numpy as np
import pandas as pd
import pytest
import torch
...
`02-cnc-machining/src/serving/__init__.py`은 완전히 빈 파일로 생성한다(내용 없음, `preprocessing`/`lstm_ae` 패키지 루트와 동일 컨벤션).

```python
# 02-cnc-machining/src/serving/inference.py
import pandas as pd
import torch
...
Run: `cd 02-cnc-machining && uv run python -c "import serving; print('ok')"`
...
**Files:**
- Create: `02-cnc-machining/src/serving/app.py`
...
```python
# 02-cnc-machining/tests/serving/test_app.py
import io
...
```python
# 02-cnc-machining/src/serving/app.py
import io
import json
...
Run: `cd 02-cnc-machining && nice -n 19 uv run uvicorn serving.app:app --reload &`

...
- `02-cnc-machining/data/rag/`(코퍼스 산출물)는 `.gitignore` 대상(`data/` 전체 관례). `02-cnc-machining/rag/sources/*.md`(원문)는 git 추적됨, 이미 존재함.
- 이번 서브프로젝트는 `src/rag/`뿐 아니라 `src/serving/`도 수정하는 서빙 파이프라인 정식 변경이라, 기존 70개 pytest 관례를 따라 정식 단위테스트를 작성한다.
- 신규 의존성(`openai`, `faiss-cpu`)은 `02-cnc-machining/pyproject.toml`에 추가.

---

## File Structure

- Create: `02-cnc-machining/src/rag/__init__.py`(빈 파일), `features.py`, `query.py`, `retrieval.py`, `generation.py`, `guide.py`
- Create: `02-cnc-machining/tests/rag/__init__.py`(빈 파일), `test_features.py`, `test_query.py`, `test_retrieval.py`, `test_generation.py`, `test_guide.py`
- Create: `02-cnc-machining/rag/build_corpus.py`
- Modify: `02-cnc-machining/src/serving/inference.py`, `02-cnc-machining/src/serving/app.py`, `02-cnc-machining/tests/serving/test_inference.py`, `02-cnc-machining/tests/serving/test_app.py`, `02-cnc-machining/pyproject.toml`

## Task 1: 의존성 추가 + `src/rag/features.py`(피처 설명 사전)

**Files:**
- Modify: `02-cnc-machining/pyproject.toml`
- Create: `02-cnc-machining/src/rag/__init__.py`, `02-cnc-machining/src/rag/features.py`
- Create: `02-cnc-machining/tests/rag/__init__.py`, `02-cnc-machining/tests/rag/test_features.py`
...
`02-cnc-machining/pyproject.toml`의 `dependencies` 배열에 다음 두 줄 추가(기존 항목들 뒤,
...
Run: `cd 02-cnc-machining && uv sync`
Expected: 에러 없이 종료, `faiss` / `openai` 패키지가 `.venv`에 설치됨

- [ ] **Step 3: 빈 패키지 초기화 파일 생성**

`02-cnc-machining/src/rag/__init__.py`: 빈 파일.
`02-cnc-machining/tests/rag/__init__.py`: 빈 파일.

- [ ] **Step 4: 실패하는 테스트 작성**

`02-cnc-machining/tests/rag/test_features.py`:
```python
...
Run: `cd 02-cnc-machining && uv run pytest tests/rag/test_features.py -v`
Expected: FAIL(`ModuleNotFoundError: No module named 'rag.features'` 또는 `ImportError`)

- [ ] **Step 6: `features.py` 구현**

`02-cnc-machining/src/rag/features.py`:
```python
...
```bash
git add 02-cnc-machining/pyproject.toml 02-cnc-machining/uv.lock 02-cnc-machining/src/rag/ 02-cnc-machining/tests/rag/
...
**Files:**
- Create: `02-cnc-machining/src/rag/query.py`
- Create: `02-cnc-machining/tests/rag/test_query.py`
...
`02-cnc-machining/tests/rag/test_query.py`:
```python
...
Run: `cd 02-cnc-machining && uv run pytest tests/rag/test_query.py -v`
Expected: FAIL(`ModuleNotFoundError`)

- [ ] **Step 3: `query.py` 구현**

`02-cnc-machining/src/rag/query.py`:
```python
...
```bash
git add 02-cnc-machining/src/rag/query.py 02-cnc-machining/tests/rag/test_query.py
...
**Files:**
- Create: `02-cnc-machining/src/rag/retrieval.py`
- Create: `02-cnc-machining/tests/rag/test_retrieval.py`
...
`02-cnc-machining/tests/rag/test_retrieval.py`:
```python
import faiss
import numpy as np
import pytest
...
Run: `cd 02-cnc-machining && uv run pytest tests/rag/test_retrieval.py -v`
Expected: FAIL(`ModuleNotFoundError`)

- [ ] **Step 3: `retrieval.py` 구현**

`02-cnc-machining/src/rag/retrieval.py`:
```python
import numpy as np
...
```bash
git add 02-cnc-machining/src/rag/retrieval.py 02-cnc-machining/tests/rag/test_retrieval.py
...
**Files:**
- Create: `02-cnc-machining/src/rag/generation.py`
- Create: `02-cnc-machining/tests/rag/test_generation.py`
...
`02-cnc-machining/tests/rag/test_generation.py`:
```python
import json
...
Run: `cd 02-cnc-machining && uv run pytest tests/rag/test_generation.py -v`
Expected: FAIL(`ModuleNotFoundError`)

- [ ] **Step 3: `generation.py` 구현**

`02-cnc-machining/src/rag/generation.py`:
```python
import json
import os
...
```bash
git add 02-cnc-machining/src/rag/generation.py 02-cnc-machining/tests/rag/test_generation.py
...
**Files:**
- Create: `02-cnc-machining/src/rag/guide.py`
- Create: `02-cnc-machining/tests/rag/test_guide.py`
...
`02-cnc-machining/tests/rag/test_guide.py`:
```python
...
Run: `cd 02-cnc-machining && uv run pytest tests/rag/test_guide.py -v`
Expected: FAIL(`ModuleNotFoundError`)

- [ ] **Step 3: `guide.py` 구현**

`02-cnc-machining/src/rag/guide.py`:
```python
...
```bash
git add 02-cnc-machining/src/rag/guide.py 02-cnc-machining/tests/rag/test_guide.py
git commit -m "Add RAG guide orchestration with good/bad branching and error handling"
```

## Task 6: `02-cnc-machining/rag/build_corpus.py` — 코퍼스 구축 + 실행

**Files:**
- Create: `02-cnc-machining/rag/build_corpus.py`

**Interfaces:**
- Consumes: `02-cnc-machining/rag/sources/sandvik_milling_troubleshooting.md`,
  `02-cnc-machining/rag/sources/osha_machine_guarding_lockout.md`(이미 존재)
- Produces: `02-cnc-machining/data/rag/corpus.json`, `02-cnc-machining/data/rag/corpus.index`
...
`02-cnc-machining/rag/build_corpus.py`:
```python
import json
...
Run: `cd 02-cnc-machining && echo ${OPENAI_API_KEY:+설정됨} && uv run python rag/build_corpus.py`
Expected: `청크 N개 저장됨`(N ≈ 20~25), `fault_category 분포`에
`vibration_backlash`/`tool_wear`/`feed_overload`/`general` 4종류가 모두 찍힘,
assertion 에러 없음, `02-cnc-machining/data/rag/corpus.json`과 `corpus.index` 파일 생성.

- [ ] **Step 3: Commit**

```bash
git add 02-cnc-machining/rag/build_corpus.py
...
**Files:**
- Modify: `02-cnc-machining/src/serving/inference.py`
- Modify: `02-cnc-machining/src/serving/app.py`
- Modify: `02-cnc-machining/tests/serving/test_inference.py`
...
`02-cnc-machining/tests/serving/test_inference.py`의 `test_predict_experiment_returns_expected_shape`
함수에서 아래 두 줄을 찾아:
```python
...
Run: `cd 02-cnc-machining && uv run pytest tests/serving/test_inference.py -v`
Expected: FAIL(`TypeError: predict_experiment() got an unexpected keyword argument 'rag_corpus'` 등)

- [ ] **Step 4: `inference.py` 수정**

`02-cnc-machining/src/serving/inference.py` 상단 import에 추가:
```python
from rag.guide import build_guide
```
...
```bash
git add 02-cnc-machining/src/serving/inference.py 02-cnc-machining/src/serving/app.py \
        02-cnc-machining/tests/serving/test_inference.py 02-cnc-machining/tests/serving/test_app.py
...
Run: `cd 02-cnc-machining && who && top -bn1 | head -6 && nice -n 19 uv run uvicorn serving.app:app --port 8899 &`
...
cd 02-cnc-machining && OPENAI_API_KEY="" nice -n 19 uv run uvicorn serving.app:app --port 8899 &
sleep 3

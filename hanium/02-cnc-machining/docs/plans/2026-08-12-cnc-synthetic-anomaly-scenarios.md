...
**Tech Stack:** Python (uv run), pandas, numpy — 기존 `02-cnc-machining` 의존성 그대로, 새 패키지 추가 없음.
...
- Create: `02-cnc-machining/synthetic/generate_synthetic.py` — perturbation 함수 3개 + `calibrate()` + `main()`
- Generated (git 포함): `02-cnc-machining/synthetic/scenarios/*.csv`, `02-cnc-machining/synthetic/scenarios/*_predict_result.json` (6개 시나리오 × 2파일 = 12개 파일)

## Task 1: `generate_synthetic.py` 작성 + 문법 확인

**Files:**
- Create: `02-cnc-machining/synthetic/generate_synthetic.py`
...
- [ ] **Step 1: `02-cnc-machining/synthetic/generate_synthetic.py` 작성**
...
Run: `cd 02-cnc-machining && uv run python -m py_compile synthetic/generate_synthetic.py`
Expected: 에러 없이 종료 (exit code 0)

- [ ] **Step 3: Commit**

```bash
git add 02-cnc-machining/synthetic/generate_synthetic.py
...
Run: `cd 02-cnc-machining && uv run python synthetic/generate_synthetic.py`
...
Run: `ls 02-cnc-machining/synthetic/scenarios/ | wc -l`
Expected: `12`
...
```bash
git add 02-cnc-machining/synthetic/scenarios/

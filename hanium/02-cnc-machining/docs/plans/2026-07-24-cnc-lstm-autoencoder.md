...
**Goal:** `02-cnc-machining/data/processed/{train,eval}.csv`(전처리 완료된 CNC 데이터)를 입력으로 받아
LSTM 오토인코더를 학습하고, 실험 단위(윈도우 오차 → 실험 단위 mean/max/95%ile 집계 →
train 분포 기반 임계값)로 양품/불량을 판정해 precision/recall을 산출하는
`02-cnc-machining/src/lstm_ae/` 패키지를 만든다.

**Architecture:** hanium 루트의 `src/lstm_ae/` 모듈 스타일(단일 책임 소파일 + 얇은
`pipeline.py` 오케스트레이터)을 `02-cnc-machining/src/lstm_ae/` 아래 독립적으로 복제한다.
...
- Python `>=3.14`, `uv_build` 백엔드. `02-cnc-machining/pyproject.toml`은 이미 존재(전처리
...
- `02-cnc-machining/` 아래 코드만 사용. hanium 루트 `src/`를 import하지 않는다(패턴만 참고, 코드
  공유 없음 — 사용자 확인된 "독립 구조" 원칙 유지).
- 새 패키지는 `lstm_ae` (`preprocessing`과 나란히 `02-cnc-machining/src/` 아래).
...
- Create: `02-cnc-machining/src/lstm_ae/__init__.py` (빈 파일)
- Create: `02-cnc-machining/src/lstm_ae/model.py`
- Create: `02-cnc-machining/tests/lstm_ae/test_model.py`
...
- [ ] **Step 1: `02-cnc-machining/pyproject.toml`에 torch 의존성 추가**
...
- [ ] **Step 2: 빈 `02-cnc-machining/src/lstm_ae/__init__.py` 생성**

- [ ] **Step 3: 실패하는 테스트 작성 — `02-cnc-machining/tests/lstm_ae/test_model.py`**
...
Run: `cd 02-cnc-machining && uv sync && uv run pytest tests/lstm_ae/test_model.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'lstm_ae.model'` (`uv sync`는 torch
설치 때문에 다소 걸릴 수 있음, 수 분 이내)

- [ ] **Step 5: `02-cnc-machining/src/lstm_ae/model.py` 구현**
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_model.py -v`
...
```bash
git add 02-cnc-machining/pyproject.toml 02-cnc-machining/uv.lock 02-cnc-machining/src/lstm_ae/__init__.py 02-cnc-machining/src/lstm_ae/model.py 02-cnc-machining/tests/lstm_ae/test_model.py
```
...
**Files:**
- Create: `02-cnc-machining/src/lstm_ae/training.py`
- Create: `02-cnc-machining/tests/lstm_ae/test_training.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/lstm_ae/test_training.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_training.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'lstm_ae.training'`

- [ ] **Step 3: `02-cnc-machining/src/lstm_ae/training.py` 구현**
...
```bash
git add 02-cnc-machining/src/lstm_ae/training.py 02-cnc-machining/tests/lstm_ae/test_training.py
```
...
**Files:**
- Create: `02-cnc-machining/src/lstm_ae/sequencing.py`
- Create: `02-cnc-machining/tests/lstm_ae/test_sequencing.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/lstm_ae/test_sequencing.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_sequencing.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'lstm_ae.sequencing'`

- [ ] **Step 3: `02-cnc-machining/src/lstm_ae/sequencing.py` 구현**
...
```bash
git add 02-cnc-machining/src/lstm_ae/sequencing.py 02-cnc-machining/tests/lstm_ae/test_sequencing.py
```
...
**Files:**
- Create: `02-cnc-machining/src/lstm_ae/scoring.py`
- Create: `02-cnc-machining/tests/lstm_ae/test_scoring.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/lstm_ae/test_scoring.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_scoring.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'lstm_ae.scoring'`

- [ ] **Step 3: `02-cnc-machining/src/lstm_ae/scoring.py` 구현**
...
```bash
git add 02-cnc-machining/src/lstm_ae/scoring.py 02-cnc-machining/tests/lstm_ae/test_scoring.py
```
...
**Files:**
- Create: `02-cnc-machining/src/lstm_ae/pipeline.py`
- Create: `02-cnc-machining/tests/lstm_ae/test_pipeline.py`
...
- [ ] **Step 1: 실패하는 테스트 작성 — `02-cnc-machining/tests/lstm_ae/test_pipeline.py`**
...
Run: `cd 02-cnc-machining && uv run pytest tests/lstm_ae/test_pipeline.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'lstm_ae.pipeline'`

- [ ] **Step 3: `02-cnc-machining/src/lstm_ae/pipeline.py` 구현**
...
```bash
git add 02-cnc-machining/src/lstm_ae/pipeline.py 02-cnc-machining/tests/lstm_ae/test_pipeline.py
```
...
**Files:**
- Create: `02-cnc-machining/scripts/run_lstm_training.py`

**Interfaces:**
- Consumes: Task 5의 `run_lstm_pipeline`, `preprocessing.columns.FEATURE_COLUMNS`(기존
  전처리 패키지, `02-cnc-machining/src/preprocessing/columns.py` — import만 하고 수정하지 않음).
- Produces: 없음 (스크립트, 실행하면 `02-cnc-machining/data/model/`에 6개 파일 생성)

- [ ] **Step 1: `02-cnc-machining/scripts/run_lstm_training.py` 작성**
...
Run: `cd 02-cnc-machining && nice -n 19 uv run python scripts/run_lstm_training.py`
...
Run: `ls -la 02-cnc-machining/data/model/`
...
```bash
git add 02-cnc-machining/scripts/run_lstm_training.py
```

(`02-cnc-machining/data/model/`은 루트 `.gitignore`의 `data` 패턴에 걸려 자동 제외됨)

---

## 완료 후 체크리스트

- [ ] `cd 02-cnc-machining && uv run pytest -v` 전체 통과 (기존 19 + 신규 8 = 27개)

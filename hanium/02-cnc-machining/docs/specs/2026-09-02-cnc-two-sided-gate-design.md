...
## 실행 결과에 따른 정정 (2026-09-02, 구현 후 라이브 재현)

Part A·B 구현(커밋 `a598558`, `cd03501`, `d883402`, 테스트 169개) 후 계획서
Task 4대로 세 시나리오를 세 프로세스로 돌렸다. champion v1(threshold 0.8566)
기준이며, 로그·`labels.db`·`requests.db` 원본은
`data/monitoring/_<시나리오>_20260902_v2/`에 보관. "기존 규칙이었다면" 열은
같은 창의 판정을 정확도 하나(후보 > champion)로 비교했을 때의 결과다.

### fixture_loosening 40일 — 5회 모두 거부, 원인 5/5 `vibration_backlash`

| 트리거 | 창 정상/불량 | 오탐 후보 vs champion | 놓침 후보 vs champion | 판정 사유 | 기존 규칙이었다면 |
|---|---|---|---|---|---|
| Day 20 (v50) | 20 / 0 | 7 vs 2 | — | 오탐 회귀 | 거부 (0.65 vs 0.90) |
| Day 25 (v51) | 20 / 0 | 9 vs 3 | — | 오탐 회귀 | 거부 (0.55 vs 0.85) |
| Day 30 (v52) | 5 / 15 | 2 vs 1 | 5 vs 8 | 오탐 회귀 (맞바꾸기) | **통과** (0.65 vs 0.55) |
| Day 35 (v53) | 0 / 20 | — | 3 vs 8 | 정상 라벨 없음 | **통과** (0.85 vs 0.60) |
| Day 40 (v54) | 0 / 20 | — | 3 vs 3 | 정상 라벨 없음 | 거부 (0.85 vs 0.85 동률) |

- **기존 규칙이었다면 이 실행에서 두 번(Day 30·35) 통과했을 것**이고, 새 규칙은
  둘 다 막았다. Day 30은 오전 재현에는 없던 사례다 — 혼합 창에서 후보가
  놓침을 3건 줄이고 오탐을 1건 늘렸는데, 정확도로는 0.65 vs 0.55로 "개선"이지만
  맞바꾸기라 거부됐다.
- 트리거 일자가 오전(19·24·29·34)보다 하루씩 늦다. `/drift-status`가 최근
  10건 요청 평균이라 feeder와 워커의 상대 속도에 따라 flagged 시작일이 하루
  흔들리는 것으로, 판정과 무관하다(08-24 기록의 같은 현상).
- champion v1 유지, 정본 해시(`9ab55583`/`6d7d3978`/`8841fd72`) 불변, 서버 로그
  에러 0.

### tool_wear 40일 — 5회 모두 거부, 원인 5/5 `tool_wear`

| 트리거 | 창 정상/불량 | 오탐 후보 vs champion | 놓침 후보 vs champion | 판정 사유 | 기존 규칙이었다면 |
|---|---|---|---|---|---|
| Day 19 (v55) | 20 / 0 | 2 vs 2 | — | 개선 없음 | 거부 (0.90 vs 0.90 동률) |
| Day 24 (v56) | 20 / 0 | 8 vs 3 | — | 오탐 회귀 | 거부 (0.60 vs 0.85) |
| Day 29 (v57) | 10 / 10 | 3 vs 4 | 5 vs 4 | 놓침 회귀 (맞바꾸기) | 거부 (0.60 vs 0.60 동률) |
| Day 34 (v58) | 0 / 20 | — | 9 vs 0 | 정상 라벨 없음 | 거부 (0.55 vs 1.00) |
| Day 39 (v59) | 0 / 20 | — | 12 vs 0 | 정상 라벨 없음 | 거부 (0.40 vs 1.00) |

- 네 가지 거부 사유(개선 없음 / 오탐 회귀 / 놓침 회귀 / 정상 라벨 없음)가
  두 시나리오에 걸쳐 전부 실제로 나왔다.
- Day 29는 Day 30(fixture)과 반대 방향의 맞바꾸기다. 기존 규칙은 동률로
  거부했지만 그건 우연이고, 새 규칙은 이유(놓침 회귀)를 가지고 거부한다.
- 거부 경로의 원인 추정 → RAG 조치 → MLflow 태그는 사유와 무관하게 10회
  모두 실행됐다. 새 run 10개(v50~v59) 전부에 `gate_g2_n_good`,
  `gate_g2_n_bad`, `gate_g2_fa_delta`, `gate_g2_miss_delta`가 있고
  `gate_g2_accuracy_delta`는 없다.

### temperature 70일 (`--pace-seconds 15`) — 거부 3회 → 통과 → 섀도우 → 승격

| 트리거 | 창 정상/불량 | 오탐 후보 vs champion | 판정 | 비고 |
|---|---|---|---|---|
| Day 22 (v60) | 20 / 0 | 7 vs 2 | 거부, 오탐 회귀 | 08-19·08-25와 같은 자리 |
| Day 27 (v61) | 20 / 0 | 7 vs 2 | 거부, 오탐 회귀 | |
| Day 32 (v62) | 20 / 0 | 11 vs 8 | 거부, 오탐 회귀 | champion도 나빠지기 시작 |
| Day 37 (v63) | 20 / 0 | 11 vs 18 | **통과 → 섀도우 시작** (관찰 기준일 Day 53) | 08-19 실측 0.45 vs 0.10과 동일한 창 |
| 섀도우 종료 Day 64 | 20 / 0 | 19 vs 20 | **승격 (v63)** | `shadow_fa_delta = -1` |
| Day 69 (v64) | 20 / 0 | 11 vs 20 (champion = v63) | 통과 → 섀도우 v64 시작, Day 70 트래픽 종료로 보류 | 승격 후 쿨다운 5일 뒤 재트리거 |

- 승격 경로는 그대로 살아 있다. 판정이 갈린 날짜와 방향이 08-19·08-25
  기록과 일치한다 — 정상 라벨뿐인 창에서 새 규칙은 정확도 규칙과 수학적으로
  같으므로 예상된 결과다.
- 섀도우 창(Day 54~57 생산분)의 오탐 19 vs 20은 `--days`가 `TOTAL_DAYS`(40)를
  넘어 변형이 설계 상한을 넘은 탓이다(08-25 섀도우 스펙의 정정 절). 두
  모델이 거의 전부 오탐인 상태에서 "1건 덜 틀렸다"로 승격됐다는 뜻이며,
  게이트 규칙이 아니라 시뮬레이션 파라미터의 한계다.
- 거부 3회에 원인 추정이 `tool_wear`로 찍혔다. 추정기는 `tool_wear` /
  `vibration_backlash` 둘만 구분하도록 설계돼 온도 드리프트는 범위 밖이다
  (08-26 스펙 비목표). 거부 경로가 사유와 무관하게 원인 추정을 실행하는 것은
  의도된 동작이고, "온도 드리프트를 세 번째 카테고리로 구분"은 별도 과제다.
- 승격으로 디스크 정본이 v63 것으로 교체됐다(`swap_with_rollback`이
  `data/model_backup/20260902_140828_229336/`에 v1 백업 생성). 재현 후
  `promote_model.py 1` + `restore_backup()`으로 되돌려 해시
  `9ab55583`/`6d7d3978`/`8841fd72` 복원 확인. champion alias → 1.

### 정리

- 오늘 아침 뚫렸던 fixture_loosening의 전부 불량인 창은 "정상 라벨 없음"으로
  거부됐고, 기존 규칙이었다면 통과했을 사례가 이 실행에서 두 건(fixture Day
  30·35) 더 있었는데 둘 다 막혔다. temperature 승격은 영향받지 않았다.
- "구현 전 데이터 확인" 절의 사후 적용 결론(뒤집히는 것은 전부 불량인 창뿐,
  temperature 불변)이 라이브에서 그대로 재현됐다. 트리거 일자가 하루 어긋난
  것은 `/drift-status` 스냅샷 타이밍 문제로 판정과 무관하다.
- 오늘 생성된 run: v50~v54(fixture), v55~v59(tool_wear), v60~v64(temperature).
  전체 테스트 169개 통과. 코드 변경 파일은 `src/retraining/gate.py`,
  `monitoring/drift_worker.py`, `tests/retraining/test_gate.py`뿐이다.

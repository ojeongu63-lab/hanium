...
- **통합 테스트의 변화 크기를 실제 운영 수준으로 줄였다(2026-09-18 오후).** 처음 값(온도 `CNC_POS_DRIFT=8`·
  `CNC_CUR_DRIFT=1.0`, 공구마모 `CNC_WEAR_RATE=20`)은 드리프트 비율이 온도 58, 공구마모 약 9,100으로 9월 2일
  실데이터 기록(40일째 온도 1.80, 공구마모 3.08, 실제 불량 실험 1.0~3.8배)과 너무 달라 결과를 보는 사람이
  신뢰하기 어렵다는 지적이 있었다. 비율이 기준선(0.8)의 두 배 이상이면 판정이 결정적이라는 계산에 따라
  온도 `CNC_POS_DRIFT=2`·`CNC_CUR_DRIFT=0.15`, 공구마모 `CNC_WEAR_RATE=0.3`으로 바꿨다.

  | 경로 | Day 3~7 비율 | 흐름 |
  |---|---|---|
  | 승격(temperature) | 1.86~2.07 | Day 5 트리거·섀도우 시작 → Day 7 v2 승격 → Day 8~12 0.36~0.49 |
  | 거부(tool_wear) | 2.51~2.71 | Day 5·7 "정상 라벨 없음" 거부, 원인 tool_wear, champion v1 유지 |

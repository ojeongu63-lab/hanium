...
- Tracking/Registry URI: `sqlite:///{ROOT}/data/mlflow/mlflow.db` (`ROOT` = `02-cnc-machining/`)
...
| 파일 | 변경 |
|---|---|
| `02-cnc-machining/src/lstm_ae/pipeline.py` | `run_lstm_pipeline()`이 학습된 `model` 객체를 반환값에 포함하도록 수정 (1줄) |
| `02-cnc-machining/scripts/run_lstm_training.py` | MLflow run으로 감싸 params/metrics/model 로깅 추가 |
| `02-cnc-machining/scripts/promote_model.py` | 신규 — champion alias 수동 지정 CLI |
| `02-cnc-machining/src/serving/inference.py` | 신규 — 모델 로드, 전처리~판정 순수 로직 |
| `02-cnc-machining/src/serving/app.py` | 신규 — FastAPI 라우팅(`/predict`, `/health`) |
| `02-cnc-machining/pyproject.toml` | `mlflow`, `fastapi`, `uvicorn` 의존성 추가 |

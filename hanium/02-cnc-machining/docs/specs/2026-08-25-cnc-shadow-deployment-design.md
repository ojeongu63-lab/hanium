...
## 실행 결과에 따른 정정 (2026-08-25)

계획대로 구현한 뒤 실제로 40일 → 55일 → 70일로 세 번 재현하며 이 스펙에
없던 문제 세 가지를 발견해 고쳤다.

**① 라벨 오프셋 버그.** `_check_shadow`가 "새로 도착한 라벨"을 개수
오프셋(`labels_seen_at_start`)으로 잘랐는데, 라벨은 생산일 기준 7일
지연 도착이라 오프셋 이후에도 여전히 섀도우 시작 전 생산분을 가리켰다.
그 배치들은 섀도우가 켜지기 전에 이미 `/predict`를 통과해 관찰 기록이
없었다. `produced_day > start_day` 필터로 수정.

**② feeder가 순식간에 끝나 섀도우가 관찰할 트래픽이 없었다.** feeder는
40~70일을 몇십 초 안에 다 쏘는데, 워커는 재학습(트리거당 실측 약 2분)
때문에 그보다 훨씬 늦게 트리거에 도달한다. 섀도우가 켜질 때쯤엔 이미
feeder가 완주해 관찰할 실트래픽이 없었다. `simulate_timeline.py`에
`--pace-seconds`(opt-in, 기본값 0)를 추가해 feeder가 워커 속도에 맞춰
쉬어가게 했다 — 이 스펙의 "`simulate_timeline.py`는 안 건드린다"는
전제를 깬 부분이다.

**③ `start_day`를 논리적 트리거일이 아니라 실제 반영 시점으로 재조회.**
①을 고친 뒤에도 여전히 매칭이 0건이었다 — 재학습 중에도 feeder는
계속 앞서가므로, 섀도우가 실제로 서버에 반영되는 순간엔 feeder가 이미
트리거 날짜보다 한참 앞선 날짜를 보내고 있었다(실측: 트리거는 "Day 37"
인데 `shadow_predictions`에 처음 기록된 건 `day48_*`). `/start-shadow`
성공 직후 `get_latest_produced_day()`를 다시 불러 그 값을 관찰 시작
기준일로 삼도록 수정.

이 세 개를 다 고친 뒤 `--days 70`, `--pace-seconds 15`로 재현해 섀도우가
실제로 끝까지(시작 → 실시간 관찰 → 라벨 매칭 → 승격) 작동하는 걸
확인했다(champion v1 → 승격 v39). 그 직후 두 가지를 추가로 발견했다:

**④ 승격 직후 쿨다운 없이 즉시 재트리거.** 섀도우 기간(37일치가 걸림)이
쿨다운(5일)보다 훨씬 길어, 섀도우가 끝났을 땐 이미 flag_history가 연속
3회를 채운 채였다. 섀도우 종료(승격이든 거부든) 시점에도
`cooldown_remaining`을 재설정하도록 수정(코드 변경, 커밋 `3bb566e`).

**⑤ `--days`가 `TOTAL_DAYS`(40)를 넘으면 변형이 설계 범위를 초과한다.**
`progress_for()`가 `TOTAL_DAYS` 기준으로 정규화되므로, `--days 70`에서
Day 60~68 구간은 progress가 1.67~1.93 — 원래 "Day 40에서 1.0"이라는
설계 상한의 거의 2배다. 그 결과 v39 승격 시 champion이 20건 중 0건을
맞혀(candidate는 0.30) 승격 판정 자체는 났지만 절대값이 정상 대역을
크게 벗어났다. **코드는 고치지 않고 알려진 한계로만 남긴다** — 섀도우
로직의 결함이 아니라 시뮬레이션 파라미터(`--days` vs `TOTAL_DAYS`) 문제이고,
이미 이 스펙의 "검증 방법" 절에 같은 종류의 이슈로 예고돼 있었다.
`--days`를 늘려 섀도우 검증 기간을 확보하려면 이 트레이드오프를
피할 수 없다 — 값을 조정해 통과시키는 대신 실측을 그대로 남긴다.
2026-09-15: `CNC_DRIFT_MAX_PROGRESS=1.0`을 주면 상한이 걸린다
(`src/config.py`). 기존 실행 명령은 그대로다.

...
### 추가 — 합성 시나리오 진폭 현실화 (2026-09-03 오후)

데모 화면에서 합성 배치의 상위 z가 만 단위(S_OutputCurrent 12,419, X_OutputCurrent 29,790)로
보여 사용자가 현실화를 요청했다. 원인은 생성기가 진폭 1.0(값 2배)에서 시작해 위로만 올린 것과,
이 센서들이 정상에서 거의 안 흔들려(학습 표준편차 4A/323A) 오차 제곱을 작은 정상 오차
표준편차로 나누는 z 계산이 곱해진 것. `synthetic/generate_synthetic.py`를 "작은 진폭에서
1.5배씩 올려 불량 판정이면서 배율 ≥ 2.0인 첫 지점"에서 멈추도록 바꾸고 다시 생성했다.

| 합성 | 진폭 전 → 후 | 배율 전 → 후 | 상위 1 z 전 → 후 | 상황 (변동 없음) |
|---|---|---|---|---|
| tool_wear | 1.0 → 0.169 | 56.2 → 2.45 | 12,419 → 298 | 확정 공구 마모 0.80 |
| feed_overload | 1.0 → 0.0675 | 597 → 3.05 | 29,790 → 107 | 확정 이송축 과부하 0.66 |
| vibration_backlash | 2.0 → 3.80 | 1.38 → 3.14 | 42 → 148 | 확정 고정구 풀림·채터 1.00 |

배경 절과 정정 절의 이전 수치(z 12418 등)는 옛 합성 배치 기준의 기록으로 남겨 둔다. 기록 4건
테스트는 그대로 통과했고 응답 예시 3개와 데모 페이지를 다시 만들었다.

### 추가 — 사용자 문구에서 "확정" 제거 (2026-09-03 오후, 팀원·멘토 피드백)

멘토 피드백: UX에서 AI가 "판단"이나 최종 확정을 하는 표현을 쓰지 말 것. 팀원 요청으로
내부 값(`confirmed` 등)은 그대로 두고 한글과 LLM 문구만 바꿨다.

| 내부 값 | 이전 `verdict_ko` | 이후 `verdict_ko` |
|---|---|---|
| confirmed | 확정 | 높은 패턴 일치 |
| composite | 복합 징후 | 복합 패턴 |
| weak | 약한 신호 | 약한 신호 |
| unknown | 판단 불가 | 일치 패턴 없음 |
| none | 이상 없음 | 이상 없음 |

프롬프트의 "시스템 판정: 확정" 줄을 "센서 패턴 대조: 높은 패턴 일치"로 바꾸고, 시스템
프롬프트에 "확정·판정 확정·원인으로 확인됨 같은 표현 금지, '센서 패턴 일치도가 높습니다' /
'관련 센서 패턴이 확인되었습니다' 수준으로 쓸 것, 일치도는 확률이 아님"을 더했다. 이전
정정 절에서 후보로 적었던 `confidence_note` 개선(일치도를 확률로 읽는 문제)이 여기서 함께
처리된다. 데모 화면의 "판정" 카드는 "센서 패턴 진단"으로. 응답 구조 11개 키는 불변이며
`verdict_ko` 값만 바뀐다.

...
스크립트. champion 모델로 실제 검증하며 생성함(진폭을 자동으로 조절 — 이상 시나리오는
작은 진폭에서 1.5배씩 올려 불량 판정이면서 점수/임계값 배율이 2.0 이상이 되는 첫 지점에서
...
도착 창에서 오탐·놓침이 둘 다 후퇴하지 않고 한쪽은 개선(G2, 정상 라벨이 없는
창은 판정 불가로 거부)을 둘 다 확인한 뒤에도 바로 승격하지 않고, 후보를 실제
